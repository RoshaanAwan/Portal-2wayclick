...
export function SystemTopbar({ user }: { user: SafeUser }) {
                    ...
                    {notifications.map((n) => {
                      const row = (
                        <div className="hover-surface flex items-start gap-2.5 px-3 py-2.5">
                          ...
                          <Avatar name={n.actorName ?? "System"} src={n.actorAvatar} size="sm" />
                          <div className="min-w-0 flex-1">
                            <p className={`text-[13px] leading-snug ${n.readAt ? "text-ink-400" : "text-ink-700"}`}>
                              {n.actorName && (
                                <span className="font-semibold text-ink">{n.actorName}</span>
                              )}{" "}
                              <span className={`font-medium ${typeColor[n.type] ?? "text-ink-500"}`}>
                                {n.message}
                              </span>
                            </p>
        ...
        <div className="relative" ref={menuRef}>
          ...
          >
            ...
            <div className="hidden text-left sm:block">
              ...
              <p className="text-[10px] leading-tight text-ink-400">System Owner</p>
            </div>
            <ChevronDown
